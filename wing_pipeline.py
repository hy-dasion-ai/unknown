...
genericThreshold = 0.0001  # might need to adjust for increased sample rate
...
def vectorize(samplerate, all_y, user, continuous_subsample_num, clip_num, time_dilation):
...
def process_single_speaker(signal, samplerate, user, clip_num, time_dilation):
    ...
    for chunk_num in range(chunk_nums):
        ...
        if test_rounding or not os.path.exists(sub_wing_plot_path):
            ...
            norms, angles = vectorize(samplerate, current_chunk, user, chunk_num, clip_num, time_dilation)
            possible_combos = set()
            for norm, angle in zip(norms, angles):
...
def write_completed_user(user, time_dilation):
...
# time_dilations = [250_000, 500_000]
# time_dilations = [1000, 2500] + list(range(5_000, 10_001, 500)) + [25_000] + list(range(50_000, 1_000_001, 50_000))
# time_dilations = [500_000]
# time_dilations = [10, 50, 100, 250, 500]
time_dilations = [100]
...
def main():
    ...
    for time_dilation in time_dilations:
        for user in user_to_filepath.keys():
            ...
            for clip_path, clip_num in filepath:
                ...
                process_single_speaker(signal, sample_rate, user, clip_num, time_dilation)

            print("Processed", user)
            write_completed_user(user, time_dilation)
